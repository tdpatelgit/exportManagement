...
class Database:
    ...
    def _migrate(self, conn=None) -> None:
        ...
        with self.get_connection() as conn:
            ...
            # v94: an export invoice can now take its Container details (11B)
            # straight off a Booking Detail, and its goods + container split
            # off a Loading Planning.
            existing = {r["name"] for r in conn.execute("PRAGMA table_info(export_invoices)")}
            ...
            if existing and "loading_planning_id" not in existing:
                conn.execute(
                    "ALTER TABLE export_invoices ADD COLUMN loading_planning_id "
                    "INTEGER REFERENCES loading_plannings(id)"
                )
