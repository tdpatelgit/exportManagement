...
SCHEMA_VERSION = 104  # v104: export_packing_list_item_designs DROPPED, and the per-line design allocation feature that wrote it is retired entirely - app/routes/export_designs_packing_lists.py, its templates, ExportPackingListService.reference_designs/save_design_allocation, ExportInvoiceRepository.sold_totals_by_design/sold_history_for_design, and the "sold" side of Inventory's stock arithmetic (InventoryService._stock_from_bought no longer subtracts sold_boxes/sold_quantity; Stock (net) is now received minus dispatched only). Superseded by tracking what has actually shipped at Packing Planning / Loading Planning instead. A real DROP TABLE, backed up first the way v55's exporters drop was.
# v103: export_designs_packing_lists DROPPED - the standalone "Designs Packing List" document (its own DSGPL number/date and printable sheet) is retired; the per-line design allocation it was built on (export_packing_list_item_designs) stayed at the time and was dropped later, in v104. A real DROP TABLE, backed up first the way v55's exporters drop was.
# v102: platform_logins (new table) - the PLATFORM LOGIN directory, a new nav item sitting right below Leads: one row per third-party platform (Facebook, IndiaMART, a marketplace portal, ...) holding its login link plus username/password/email/mobile, so any teammate can jump straight into the account instead of hunting down a password. Company-scoped, no satellites, same plain admin-gated-write CRUD as transporters (v38). The table is brand new, so schema.sql creates it on old and fresh databases alike and _migrate needs no step.
# v101: quotation_items.packing_unit + proforma_invoice_items.packing_unit (new nullable TEXT) - the Packing dropdown's "Manual - Packing" option now also picks WHICH unit the typed figure counts (PLT / CTN / BOX / BAG / PCS / SET), printed after it on the sheet; NULL on every existing row reads as PLT, exactly as before.
...
class Database:
    ...
    def _migrate(self, conn=None) -> None:
        ...
        with self.get_connection() as conn:
            ...
            # v103: export_designs_packing_lists DROPPED - the standalone
            # Designs Packing List document is retired. Backed up first, same
            # treatment v55's exporters drop got.
            tables = {r["name"] for r in conn.execute(
                "SELECT name FROM sqlite_master WHERE type='table'")}
            if "export_designs_packing_lists" in tables:
                conn.commit()  # so the copy below is a consistent snapshot
                self._backup_db_file("pre_v103_drop_export_designs_packing_lists")
                conn.execute("DROP TABLE export_designs_packing_lists")

            # v104: export_packing_list_item_designs DROPPED - the per-line
            # design allocation feature (which designs a container's boxes
            # actually are) is retired entirely, along with the "sold" side
            # of Inventory's stock arithmetic it fed. Backed up first, same
            # treatment v55's exporters drop got.
            tables = {r["name"] for r in conn.execute(
                "SELECT name FROM sqlite_master WHERE type='table'")}
            if "export_packing_list_item_designs" in tables:
                conn.commit()  # so the copy below is a consistent snapshot
                self._backup_db_file("pre_v104_drop_export_packing_list_item_designs")
                conn.execute("DROP TABLE export_packing_list_item_designs")
